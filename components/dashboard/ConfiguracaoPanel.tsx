  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch('/api/configuracoes', {
        ...
        body: JSON.stringify({ campanha_inicio: inicio, campanha_fim: fim, meta_diaria_apm: meta }),
      });
      if (!res.ok) {
        const err = await res.json();
        setMsg({ ok: false, text: err.error ?? 'Erro ao salvar.' });
      } else {
        const nova: CampanhaConfig = { campanha_inicio: inicio, campanha_fim: fim, meta_diaria_apm: meta };
        onConfigChange(nova);
